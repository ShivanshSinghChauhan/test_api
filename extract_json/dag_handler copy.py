...
from app_schemas import DagSchema, OperatorParameterSchema
    ...
    def load_from_links(links, task_name_mappings):
        ...
        for link_dict in links:
            try:
                ...
                if link_dict["fromNode"] == "out_port":
                    from_dict = link_dict["fromNode"]
                    to_dict = link_dict["toNode"]
                else:
                    from_dict = link_dict["toNode"]
                    to_dict = link_dict["fromNode"]
                    

                graph.add_edge(
                    #task_name_mappings[from_dict["nodeId"]],
                    #task_name_mappings[to_dict["nodeId"]],
                    task_name_mappings[from_dict],
                    task_name_mappings[to_dict],
                )
